    ...
    <form onSubmit={handleSubmit} className='flex flex-col gap-3 mb-12'>
        ...
        {tags?.length > 0 && (
          <div>
            ...
            <ul>
            {tags.map((tag, index) => (
                <li key={`${tag}-${index}`} className='border-b py-6'>
                  <span className='text-xl mt-3'>
                    {tag}
                  </span>            
                  <button type="button" onClick={() => handleRemoveTag(tag)} className={`mt-3 w-[fit-content] flex items-center gap-x-3 bg-[#ed9043] text-white hover:bg-black duration-150 py-[11px] sm:py-[12px] px-5  rounded uppercase text-[11px] sm:text-xs font-[500] tracking-[1px]`}>
